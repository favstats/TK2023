...
```{r, out.width="100%"}
more_data <- readr::read_rds("https://github.com/favstats/meta_reports2/raw/main/lifelong/NL.rds")  %>%
...
hc_data %>%
  ...
  mutate(`Daily Spend (cumulative)` = spend - lag(spend, default = first(spend))) %>%
  ungroup()  %>%
  # mutate(spend)
