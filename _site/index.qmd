...
```{r, out.width="100%"}
more_data <- readr::read_rds("https://github.com/favstats/meta_reports2/raw/main/daily/NL.rds")  %>%
  ...
  filter(cntry == "NL") %>% 
  filter(date_produced >= lubridate::as_date("2023-07-07"))
...
tmin_date <- min(hc_data$date_produced, na.rm = TRUE)
tmax_date <- max(hc_data$date_produced, na.rm = TRUE)
hc_data %>%
  group_by(party, colors) %>% 
  arrange(date_produced) %>% 
  mutate(Date = date_produced) %>%
  complete(Date = seq.Date(min(tmin_date), max(tmax_date), by="day")) %>%  # Create rows for missing dates
  mutate(spend = ifelse(is.na(spend), 0, spend)) %>% 
  mutate(spend2 = cumsum(spend)) %>% 
  fill(spend, .direction = "down") %>%
  mutate(`Daily Spend (cumulative)` = spend2) %>%
  # mutate(spend)
  hchart("area", hcaes(x = Date, y = `Daily Spend (cumulative)`, group = party, fill = colors)) %>%
