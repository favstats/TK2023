...
MOST_SPENT <- vvd_dat %>% 
  ...
  select(pc4_code, party, total_spend_old = total_spend) %>%
  bind_rows(cda_dat %>% select(pc4_code, party, total_spend_old = total_spend, gem_name)) %>%
  bind_rows(gl_dat %>% select(pc4_code, party, total_spend_old = total_spend, gem_name)) %>%
  bind_rows(d66_dat %>% select(pc4_code, party, total_spend_old = total_spend, gem_name)) %>%
  ...
  # bind_rows(d66_dat %>% select(pc4_code, party, total_spend_old, gem_name)) %>%
  select(pc4_code, party, total_spend_old, gem_name) %>% 
  group_by(pc4_code) %>% 
...
geo %>%
    left_join(all_zip %>% 
                filter(!(party %in% c("BBB", "Ja21", "Libertaire Partij"))) %>% 
  group_by(pc4, party) %>% 
  summarize(total_spend = sum(total_spend_old)) %>% 
  arrange(desc(total_spend)) %>% 
  left_join(total_by_all) %>% 
  mutate(perc = total_spend/total_spend_formatted) %>% 
  ungroup() %>% 
  group_by(pc4) %>% 
  summarize(total_spend = mean(perc*100)) %>% 
  arrange(desc(total_spend))  %>% 
    dplyr::mutate(pc4_code = as.character(pc4))) %>%
  ggplot()  +
  theme_void() +
  # geom_sf(color = "black") +
  # geom_sf(aes(fill = log(total_spend)+2),
...
geo %>%
    left_join(all_zip %>% 
                filter(!(party %in% c("BBB", "Ja21", "Libertaire Partij", "GroenLinks-PvdA"))) %>% 
    dplyr::mutate(pc4_code = as.character(pc4)) %>% 
  group_by(pc4_code) %>% 
  summarize(total_spend = median(total_spend_old)) %>% 
  arrange(desc(total_spend))) %>% 
  # left_join(total_by_all) %>% 
  # mutate(perc = total_spend/total_spend_formatted) %>% 
  # ungroup() %>% 
  ...
  theme(legend.position = "bottom") +
  scale_fill_viridis_c() +
  guides(fill = guide_colourbar(title = "Median Spend (€)", barwidth = 10, barheight = 0.5))
...
geo %>%
  ...
  # arrange(desc(total_spend))  %>% 
    # dplyr::mutate(pc4_code = as.character(pc4))) %>%
  mutate(total_spend = total_spend/inwoner*1000) %>% 
  # mutate(total_spend = ifelse(total_spend >= 10000, 10000, total_spend)) %>% 
  mutate(total_spend = ifelse(total_spend >= 250, 250, total_spend)) %>% 
  ggplot()  +
  theme_void() +
  # geom_sf(color = "black") +
  # geom_sf(aes(fill = log(total_spend)+2),
  #         colour = "white", size = 0.9) +
  # geom_sf(aes(fill = party, alpha = total_spend_old),
  geom_sf(aes(fill = total_spend, alpha = total_spend),
          colour = NA,
          size = 0.00000001) +
  scale_alpha(range = c(0.5, 1)) +
  # geom_sf(colour = "white", fill = NA, alpha = 0.001,
...
geo %>%
  ...
  # arrange(desc(total_spend))  %>% 
    # dplyr::mutate(pc4_code = as.character(pc4))) %>%
  mutate(total_spend = ifelse(total_spend >= 500, 500, total_spend)) %>% 
  ggplot()  +
  theme_void() +
  # geom_sf(color = "black") +
  # geom_sf(aes(fill = log(total_spend)+2),
...
```{r}
# well actually you just need this code:
...
all_zip %>%
  ...
  summarize(
    ...
    median_spend = median(total_spend_old),
    inwoner = inwoner[1]
  ) %>%
  ungroup() %>%
  ## spending per inhabitants
  mutate(total_spending_per_inhabitants = total_spend / inwoner * 1000) %>%
  ## get rid of outliers
  mutate(
    total_spending_per_inhabitants = ifelse(
      total_spending_per_inhabitants >= 250,
      250,
      total_spending_per_inhabitants
    )
  ) 
```
...
```{r}
 all_zip %>%
  ...
  filter(party!="Ja21") %>% 
    right_join(cbs %>%
    janitor::clean_names()) %>% 
   mutate(ignored = ifelse(!is.na(party), F, T)) %>% #View()
   distinct(pc4, .keep_all = T) %>% 
  filter(inwoner>= 0) %>% 
   ggplot(aes(ignored, inwoner)) +
   geom_boxplot() +
   scale_y_log10() 
 ...
 all_zip %>%
  ...
  filter(party!="Ja21") %>% 
    right_join(cbs %>%
    janitor::clean_names()) %>% 
   mutate(ignored = ifelse(is.na(party), T, F)) %>% #View()
   distinct(pc4, .keep_all = T) %>% 
  # filter(inwoner>= 0) %>% 
  filter(p_nw_mig_a >=0) %>%
   ggplot(aes(ignored, p_nw_mig_a)) +
   geom_boxplot() +
   EnvStats::stat_mean_sd_text() +
   ggpubr::stat_compare_means() +
   theme_minimal() +
   labs(y = "% Non-Western migration background") +
 ...
 all_zip %>%
  ...
  # filter(inwoner>= 0) %>% 
  filter(inw_65pl >=0) %>% 
   ggplot(aes(ignored, inw_65pl/inwoner*100)) +
   geom_boxplot() +
   EnvStats::stat_mean_sd_text() +
   ggpubr::stat_compare_means() +
   theme_minimal() +
  ...
  # filter(inwoner>= 0) %>%     
    ...
    mutate(uitkminaow = (uitkminaow/inwoner*100)) %>%
   ggplot(aes(ignored, uitkminaow)) +
   geom_boxplot() +
   EnvStats::stat_mean_sd_text() +
   ggpubr::stat_compare_means() +
   theme_minimal() +
   labs(y = "% of people that receive social benefits") +
   EnvStats::stat_n_text() 
```
...
```{r}
 all_zip %>%
  ...
  filter(party!="Ja21") %>% 
    right_join(cbs %>%
    janitor::clean_names()) %>% 
   mutate(ignored = ifelse(!is.na(party), F, T)) %>% 
  filter(ignored) %>% 
  arrange(desc(inwoner)) %>% 
  select(-pc4_code, -total_spend_old) #%>% 
  # openxlsx::write.xlsx("../data/ignored.xlsx")


targeted_top <- all_zip %>%
  mutate(pc4_code = str_remove_all(value, ", Netherlands")) %>%
    select(pc4, pc4_code, total_spend_old, party)  %>% 
  # filter(party!="GroenLinks-PvdA") %>%
  # filter(party!="BVNL") %>% 
...
all_zip %>% 
  group_by(pc4) %>% 
  summarize(total_spend_old = sum(total_spend_old),
            total_spend = sum(total_spend)) %>% 
  right_join(targeted_top) %>% 
  arrange(desc(total_spend_old)) %>% 
  mutate(price_per_thousand = total_spend/inwoner*1000) %>% 
  select(pc4, total_spend_old, price_per_thousand, everything())%>% 
  arrange(desc(price_per_thousand)) %>% 
  ggplot(aes(price_per_thousand, uitkminaow/inwoner)) +
  geom_point() +
...
all_zip %>% 
  ...
  select(pc4, total_spend_old, price_per_thousand, everything())%>% 
  arrange(desc(inwoner)) %>% 
  openxlsx::write.xlsx("../data/targeted.xlsx")
```
...
cbs %>%
  ...
  mutate(pc4 = pc4_code) ) %>% 
  filter(inwoner >= 0) %>% 
  ggplot(aes(inwoner, n)) +
  geom_jitter(height = 0.6) +
  stat_cor() +
  geom_smooth(method = "lm") +
  scale_x_log10() +
  labs(y = "Number of Parties Targeting Postcode") +
...
cbs %>%
  ...
  mutate(pc4 = pc4_code)) %>% 
  filter(p_nw_mig_a >=0) %>%
  ggplot(aes(p_nw_mig_a, n)) +
  # filter(p_nl_achtg >=0) %>% 
  # ggplot(aes(p_nl_achtg, n)) +
  geom_jitter() +
  stat_cor() +
  geom_smooth(method = "lm")# +
```
...
election_dat30 %>% 
  filter(str_detect(value, "1781")) %>% View()
```
